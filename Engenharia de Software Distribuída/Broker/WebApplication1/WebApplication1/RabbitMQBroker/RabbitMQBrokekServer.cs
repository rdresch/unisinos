...
{
    public class RabbitMQBrokerServer
    {
        ...
        public static void StartConsumer()
        {
            ...
            var channel = CreateChannel(connection, SERVER_QUEUE_NAME);
            channel.QueueBind(SERVER_QUEUE_NAME, SERVER_EXCHANGE_NAME, ROUTING_KEY, null);
            ...
            consumer.Received += (model, ea) =>
            {
                ...
                AddMessage($"Message received from client: {message}");
        ...
        private static IModel CreateChannel(IConnection connection, string queueName)
        {
            ...
            channel.ExchangeDeclare(SERVER_EXCHANGE_NAME, ExchangeType.Direct, true, true, null);
        ...
        private static void SendMessageToClient(string message)
        {
            ...
            using (var channel = CreateChannel(connection, CLIENT_QUEUE_NAME))
            {
                channel.BasicPublish(
                    exchange: CLIENT_EXCHANGE_NAME,
                    routingKey: ROUTING_KEY,
                    basicProperties: null,
                    body: Encoding.UTF8.GetBytes(message));

                AddMessage($"Message sent to client: {message}");
            }
        }
